...
- **事実と推測を混ぜない**: 確認した事実と推測・未検証の内容を断定として混ぜない。推測は「推測」「未確認」と明示し、検証できるものは検証してから述べる。計画 doc・技術的主張を書き出す前に前提を列挙し、検証状況（確認済み / 未確認）を明記する（`plan-doc` template の「前提と検証状況」セクション）。
...
- **push 前にレビュー必須（docs のみの差分は除く）**: push 前に `/code-review` と `/security-review` を通す。**差分が `*.md` / `docs/` / `LICENSE` だけなら両方とも不要**（散文はビルドを壊さず脆弱性も入らない）。コード・設定・workflow・lockfile が1つでも含まれるなら必須。効かせる強さは差分量で決める——lockfile を除いた変更が 100 行未満なら `/code-review low`、それ以上なら `/code-review`（**行数で skip 判断はしない**。理由: 小さな差分こそ重大なバグが潜む。実例として、テスト 30 行の追加で CI が 6 時間ハングするバグが code-review で見つかっている）。Claude Code では PreToolUse hook（`pre-push-review-gate.sh`）がこの判定を自動で行い、ブロック時に実行すべきコマンドを提示する。他 CLI では手動で同じ基準を適用する。
...
- **GitHub 参照は `gh` コマンド優先**: WebFetch は長文で取得漏れが起きるため、`gh` で取れない情報のみ WebFetch にフォールバックする。Claude Code では PreToolUse hook（`webfetch-github-gate.sh`）が github.com 系ホストへの WebFetch をブロックし、対応する `gh` コマンドを提示する。
...
- **playwright / chrome-devtools でブラウザを開いたら、そのタスク内で必ず閉じる**: 用が済んだら `browser_close` を呼ぶ。委譲した subagent が開く場合は「最後に閉じる」を委譲プロンプトにも書く（閉じるのは起動した側の責任で、main からは観測できない）（理由: playwright MCP は専用 `--user-data-dir` で別インスタンスの Chrome を起動するため、残留すると同一バンドル宛の Apple Events がそちらへ向き、**エラーではなく空の結果**として後続タスクを壊す。実例: 翌日の `osascript` が `count of windows` に 0 を返し、Chrome 未起動と誤認しかけた。判別は `pgrep -f "ms-playwright-mcp/mcp-chrome"`）。
...
- **permission でブロックされた操作は回避しない**: deny されたコマンド（`rm` 等）は代替手段で迂回せず、実行すべき正確なコマンドをコードブロックで提示してユーザーの手動実行に委ねる。提示後は残りの作業を続行する。
...
- **英語の専門用語には初出時に日本語の一言説明を付ける**（例:「recall（取りこぼしの少なさ）」）。ツール名・コマンド名はそのままで可。用語を英語のまま流して説明を続けない。
- **仕組みを提案する前に、それが何かを1行で説明する**。sandbox / hook / MCP のような機構は「何をするものか」を先に言ってから提案に入る。
- **疑問形は質問であって指示ではない**。「〜なの？」「〜だっけ？」にはまず答える。作業に移すのは明示的な指示があってから。
- **短時間の連投は1つの思考の推敲として扱う**。最後のメッセージを基準にまとめて解釈する。typo・IME 変換事故は文脈から意図を推定し、解釈を1行添えて進める（確認で止めない）。
- **同一セッション内で矛盾する説明をしない**。既に答えた質問に再度触れるときは前の回答と整合させるか、誤りなら誤りと明示して訂正する。
