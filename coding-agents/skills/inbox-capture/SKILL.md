...
- **セクション見出しは `title` 列をそのまま使う。記事タイトルを自分で作文しない**（HTML はページの `<title>`、それ以外は本文の先頭見出し）。`title` が空のときのフォールバック: **PDF** は1ページ目の表題、**HTML / GitHub** は本文の先頭見出し。要約は出力ファイル群を **Read で並列に読んで**行う（PDF は Read の `pages` 引数。10 ページ超は `pages` 必須・1回最大20。全ページは読まない）。
...
| `empty` | 取得はできたが本文が空（JS 描画 SPA） | **playwright / chrome-devtools でスナップショットを取り自分で抽出・要約**（`.playwright-mcp/` に残るが gitignore 済み。使ったら Phase 0 のとおり `browser_close`）。WebFetch は playwright も使えないときの最終手段 |
...
- 追記: `update-note`。引数は `_id` と `_rev`（`noteId` ではない。`note-head.sh` の `id` / `rev` をそのまま使う）。**既存本文を絶対に消さず末尾にセクションを追加するだけ**。
  ...
  - **連結先に `reading-time.sh apply` をかけてから `jq` に渡す**（新規作成時も同じ）。既存本文を文脈に載せずに冒頭行だけ書き換えられる唯一の経路。詳細は「推定読了時間」。
...
- **タグは付けない**（週次で「育てる」判定して昇格したときだけ）。日次ノートの既読管理も**タグではなく `status`** で行う——ユーザーがその日の分に目を通したら `completed` にする運用なので、作成時の `active` は「未読」を意味する。タグを状態管理に使うと `notebooklm-judge` の全タグ走査に偽の昇格候補として出てくる。
...
- 冒頭の `> 推定読了時間:` 行は**手書きせず `reading-time.sh` に生成・更新させる**（下記「推定読了時間」）。
...
- `### メモ` は**ユーザー専用欄**。取得上の注記・関連づけ・読了判定の理由などをClaudeが書き込むことはせず、常に空欄で出力する。人間が後から自由に書き込む欄なので、**追記時に既存のメモを消さない**。
...
1. **深読みして知識化したいか？** → Yes なら**分量に関わらず「後で読む」**。「誰の関心か」の参照元は、作業中リポジトリの `CLAUDE.md` と `.claude/plans/` の進行中タスク、既存 inbox で「後で読む」が付いた記事の傾向。推定であり誤ることもあるが、`### メモ` には書かず判定結果だけ残す（週次で人間が覆せる）。
...
ノート冒頭（`# inbox YYYY-MM-DD` の直下）に、その日の分を読み切るのにかかる推定時間を 1 行置く。**対象は「ノートの要約テキスト」+「即読了以外（要約で十分 / 後で読む）の記事本文」**——即読了は日次の時点で読み終わっているので残り時間に数えない。Phase 2 の連結後・MCP 送信前に刻む。

1. 要約分: **今回書く新セクションだけ**を渡す → `~/dotfiles/coding-agents/skills/inbox-capture/scripts/reading-time.sh estimate <新セクション.md>`
2. 記事分: **即読了以外の** `NNN.md` / `NNN.pdf` だけを渡す → 同 `estimate <files...>`
3. 刻む: 同 `apply --summary <分> --articles <分> --count <即読了以外の本数> <連結先>`（更新後の行を標準出力に返す）

- **`apply` は既存行に加算する（置換しない）**。同日 2 回目以降も渡すのは**今回増えた分だけ**でよい。行が無ければ `# ` 見出しの直下に挿入する。
- 速度は CJK 500 字/分・英文 250 語/分・PDF 2 分/ページ（`-p` で変更）。URL は数えない。時間は切り上げ。
- **PDF のページ数は `pdfinfo`（poppler）で数える**。`mdls` は `$TMPDIR` が Spotlight 索引外で答えず、生バイト走査は PDF 1.5+ の圧縮ページツリーを取りこぼすので、どちらも fallback。**全滅したら 0 分にせず exit 2 で落ちる**（黙って合計から消えるより落ちる方が良い）。`brew install poppler`。
- 渡すファイルが 0 件でもエラーにならず `0` を返す（全部「即読了」の日・全件取得失敗の日）。
- 取得失敗した記事は本文が無いので記事分にも `--count` にも入れない（`## 取得失敗` に残るので欠落は追える）。
- 回帰テストは `scripts/test-reading-time.sh`（オフライン・認証なし。`TMPDIR` が書けない環境は `TEST_TMPDIR=<書ける場所>`）。
