   ...
   - 系統A（fetch+要約）: URL群を 5〜10 件ずつバッチ処理して fetch → 要約 → 読了判定。
     - **取得方法は URL のホスト「ファミリ」で分岐する**（特定パターンの列挙ではなく分類で判断する。列挙だと未掲載のサブパスが未定義になる）:
       ...
       - **その他（HTML 記事）**（zenn / Qiita / speakerdeck / ニュース等）は **`ax`**（`ax <URL> --md` で本文を markdown 取得。CLAUDE.md「Web 取得・HTML 抽出は `ax` を既定にする」の使用地点側の徹底）。WebFetch は ax で取れないときのフォールバック。
         - **心構え: `ax` は "AI 時代の curl"。取得・抽出の層であって要約はしない**（要約するのは呼び出した自分。WebFetch のように弱いモデルが要約して返すのとは別レーン）。だから `curl` の出力を `head -c` で切らないのと同じで、**`ax --md` の出力を `head -c` 等の外部 cap で切らない**——切ると記事の結論（末尾に多い）が読めず、読了判定を誤る。
         - `ax --md` は既定で出力が capped され末尾に「N more hidden — `--all` / `--offset` / `--budget`」と出る。長さは **ax 自身の `--budget <T>` で、記事全体（結論まで）が入る大きさに取る**。ax が「多数 hidden」と報告する長文は、冒頭だけで読了判定せず `--all` か `--offset` で末尾も取る。
     ...
     - **文脈汚染を避ける（系統A は件数を問わず subagent に隔離）**: `ax --md` / `gh` は生本文を丸ごと返し、一度 main の tool result に乗れば要約後もログに残り続ける＝**inline fetch は件数によらず必ず汚染する**。inbox の 系統A は「生本文を読む → 要約 → 生本文は捨てる」で **main が生本文を後で使うことは一切ない**（生本文は main にとって純粋なゴミ）ので、**件数を問わず 系統A ごと subagent（`general-purpose`）に隔離**し、main は完成した各セクション（要約 + 読了判定）だけ受け取る（`parallelization` skill「大量探索は subagent、main は要約だけ」の適用）。subagent 側は文脈が使い捨てなので `--budget` を大きく取り**全文（結論まで）を読める**。※例外は 1〜2 件だけの単発時——subagent の spawn overhead が汚染より重いと判断すれば main 直も取り得るが、それは「少量だが汚染する」ことを承知した妥協であって clean ではない（既定はあくまで隔離）。
