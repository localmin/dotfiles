...
5. **push 前ゲート**: `/code-review` と `/security-review` を通し findings に対処する。
   ...
   - **指摘への対処で commit すると gate が再武装する**。HEAD が変わるたびにレビューが
     無効化されるので、機械的に回し続けると「直す→再レビュー→また軽微な指摘」で終わらない。
     **指摘の質が実害のあるバグからコメント文言・型の精密化まで下りたら、そこで止めて
     残りを報告し、push するか続けるかをユーザーに判断させる**（理由: 実質的なレビューは
     既に済んでおり、以降の周回は費用と時間を消すだけ。実例として同一ブランチで full review を
     4巡させ、spend limit に2回当てた）。
